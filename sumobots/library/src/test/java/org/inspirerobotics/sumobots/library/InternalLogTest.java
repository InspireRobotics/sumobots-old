...
public class InternalLogTest {
	...
	public void logLinesClearTest() {
		...
		Assert.assertTrue("After a clear the internal log still wasn't empty!!",
				InternalLog.getInstance().getLogLines().isEmpty());
	}
	...
	public void loggerResetTest() {
		...
		Assert.assertTrue("After reset the internal log wasn't empty!!",
				InternalLog.getInstance().getLogLines().isEmpty());
	}
